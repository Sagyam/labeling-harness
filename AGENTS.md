...
- **No route diarizes** (D52, reversing D49), so `hypothesis_words.speaker` is null for anything
  ...
  turns cannot be trusted, neither for who said a word nor for where a turn changes (D100): the
  owner heard them fail, and in crosstalk the per-word join decides by a tie rule, not by the audio. Rows
